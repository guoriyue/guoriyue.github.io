...
test('stays inside viewport including windows smaller than the sprite', () => {
  assert.deepEqual(clampPoint({x:999,y:-4},320,200),{x:292,y:0});
...
test('reaches the cursor offset and stops without overshooting in either direction', () => {
  for (const target of [{x:300,y:180},{x:0,y:0}]) {
    let position={x:100,y:80};
    let next;
    for(let i=0;i<600;i++) {
      next=advanceCollie(position,target,16);
      assert.ok(Math.hypot(next.x-target.x,next.y-target.y)<=Math.hypot(position.x-target.x,position.y-target.y));
